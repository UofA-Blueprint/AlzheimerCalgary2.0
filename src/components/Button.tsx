...
const Button: React.FC<ButtonProps> = ({
  ...
  const buttonClassName = `flex flex-row items-center justify-center p-4 ${
    status === "disabled"
      ...
      : color
      ? `relative overflow-hidden text-${color} border-${color} border-2 active:bg-white active:text-${color} transition-colors before:absolute before:left-0 before:top-0 before:-z-10 before:h-full before:w-full before:origin-top-left before:scale-x-0 before:bg-${color} before:transition-transform before:duration-300 before:content-[''] hover:text-white before:hover:scale-x-100`
