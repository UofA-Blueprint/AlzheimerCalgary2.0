...
export default function MemberPage() {
	...
	return (
		<div className="font-display relative">
			...
			<NavigationBar userType="user" className="sticky top-0 left-0" />
            ...
            <div className="flex flex-col space-y-4 py-6">
					{userData && (
						<MemberHeader
							...
							profilePicChildren={
								<img className="w-32 h-32 object-cover rounded-full" src={userData.profilePicture?.src} alt="Profile pic" />
							}
						/>
					)}
				</div>
				...
				<div className="min-h-screen pt-2 flex flex-col">
                    <div className = "w-56 left-12 pb-4  place-self-end">
                        {/* temporary */}
                        <SortDropdownList onSelect={handleSortSelect}></SortDropdownList>                        
                    </div>
                    {/* MediaGrid component to display sorted media items */}
					<MediaGrid data={data} sortOrder={sortOrder} />
				</div>
			</div>
		</div>
	);
